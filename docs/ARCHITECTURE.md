...
System integrations are capabilities, not startup requirements. Missing
Accessibility, Automation, private Launch Services symbols, a readable Dock
plist, or an optional application icon must not prevent configuration access or
custom-slot launching.

The private Launch Services adapter dynamically resolves symbols and returns
an empty result when they are absent. It is isolated to badge/application
