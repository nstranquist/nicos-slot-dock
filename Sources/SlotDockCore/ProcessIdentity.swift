...
/// Pure process-identity rules for Nicos Slot Dock (used by products runtime match + tests).
...
public enum SlotDockProcessIdentity {
    ...
    public static let installedAppName = "Nicos Slot Dock.app"
    public static let legacyInstalledAppName = "Slot Dock.app"

    /// Path suffixes / basenames that identify a live Nicos Slot Dock process in `ps` output.
    public static var processMatchKeys: [String] {
        [
            ...
            "\(legacyInstalledAppName)/Contents/MacOS/\(executableName)",
        ]
    }

    /// Whether a process command line is Nicos Slot Dock.
    /// Uses the full command string so paths with spaces (`Nicos Slot Dock.app`) still match.
